...
public:
	...
	/// @brief The position p satisfies
	///				p >= 0 && p < size()
	///			If p is invalid, erase throws an exception
	///			erase(p) removes the value at position p, as well as releasing
	///			the memory for that value/node
	/// @param p position/index of the value to be removed
	...
	/// The positions / values in the sequence at
